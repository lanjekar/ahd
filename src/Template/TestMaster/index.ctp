 ...
 * @var \App\Model\Entity\TestMaster[]|\Cake\Collection\CollectionInterface $testMaster
 */
?>
...
<div class="testMaster index large-9 medium-8 columns content">
    <h3><?= __('Test Master') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($testMaster as $testMaster): ?>
            <tr>
                <td><?= $this->Number->format($testMaster->test_list_id) ?></td>
                <td><?= h($testMaster->test_name) ?></td>
                <td><?= $this->Number->format($testMaster->is_active) ?></td>
