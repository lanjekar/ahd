 ...
 * @var \App\Model\Entity\DistrictMaster[]|\Cake\Collection\CollectionInterface $districtMaster
 */
?>
...
<div class="districtMaster index large-9 medium-8 columns content">
    <h3><?= __('District Master') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($districtMaster as $districtMaster): ?>
            <tr>
                <td><?= $this->Number->format($districtMaster->district_id) ?></td>
                <td><?= $districtMaster->has('state_master') ? $this->Html->link($districtMaster->state_master->state_id, ['controller' => 'StateMaster', 'action' => 'view', $districtMaster->state_master->state_id]) : '' ?></td>
                <td><?= h($districtMaster->district_name) ?></td>
                <td><?= h($districtMaster->is_active) ?></td>
