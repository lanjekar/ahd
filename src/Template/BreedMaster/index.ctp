 ...
 * @var \App\Model\Entity\BreedMaster[]|\Cake\Collection\CollectionInterface $breedMaster
 */
?>
...
<div class="breedMaster index large-9 medium-8 columns content">
    <h3><?= __('Breed Master') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($breedMaster as $breedMaster): ?>
            <tr>
                <td><?= $this->Number->format($breedMaster->breed_id) ?></td>
                <td><?= $breedMaster->has('species_master') ? $this->Html->link($breedMaster->species_master->species_id, ['controller' => 'SpeciesMaster', 'action' => 'view', $breedMaster->species_master->species_id]) : '' ?></td>
                <td><?= h($breedMaster->breed_name) ?></td>
                <td><?= $this->Number->format($breedMaster->is_active) ?></td>
